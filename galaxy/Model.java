...
class Model {
    ...
    private int b;
    /** The board storing the centers. */
    private Place[][] board;
    /** The board storing boundaries. */
    private boolean[][] boundaryboard;
    /** The board storing marks. */
    private int[][] markboard;
    /** The list storing centers. */
    private ArrayList<Place> centerslist;
    ...
    void copy(Model model) {
        if (model == this) {
            ...
            this.numcol = model.numcol;
            this.numrow = model.numrow;
            this.a = model.a;
            this.b = model.b;
            this.board = new Place[a][b];
            for (int i = 0; i < model.board.length; i++) {
                System.arraycopy(model.board[i], 0,
                        this.board[i], 0, this.board[0].length);
            }
            this.boundaryboard = new boolean[a][b];
            for (int i = 0; i < model.boundaryboard.length; i++) {
                System.arraycopy(model.boundaryboard[i], 0,
                        this.boundaryboard[i], 0, this.boundaryboard[0].length);
            }
            this.centerslist = new ArrayList<Place>();
            for (Place s: model.centerslist) {
                this.centerslist.add(s);
            }
            this.markboard = new int[a][b];
            for (int i = 0; i < model.markboard.length; i++) {
                System.arraycopy(this.markboard[i], 0,
                        markboard[i], 0, this.markboard[0].length);
            }
    ...
    void init(int cols, int rows) {
        ...
        b = (numrow * 2) + 1;
        board = new Place[a][b];
        boundaryboard = new boolean[a][b];
        for (int i = 0; i < a; i++) {
            for (int k = 0; k < b; k++) {
                if (i == 0 || k == 0 || i == a - 1 || k == b - 1) {
                    boundaryboard[i][k] = true;
                }
            }
        }
        centerslist = new ArrayList<Place>();
        markboard = new int [a][b];
    }
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (isCenter(boundary) && boundary != center) {
                    return false;
                }
            }
            for (int i = 0; i < 4; i += 1) {
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        if (isIntersection(center)) {
            ArrayList<Place> intersection = new ArrayList<>();
            intersection.add(center.move(-1, 1));
            intersection.add(center.move(+1, 1));
            intersection.add(center.move(+1, -1));
            intersection.add(center.move(-1, 1));
            for (Place s: intersection) {
                accreteRegion(s, galaxy);
            }
        } else if (isCell(center)) {
            accreteRegion(center, galaxy);
        } else if (isCell(center.x - 1, center.y)) {
            accreteRegion(pl(center.x - 1, center.y), galaxy);
        } else if (isCell(center.x + 1, center.y)) {
            accreteRegion(pl(center.x + 1, center.y), galaxy);
        } else if (isCell(center.x, center.y - 1)) {
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        markAll(region, 1);
        while (unmarkedSymAdjacent(center,
                new ArrayList<Place>(region)).size() != 0) {
            region.addAll(unmarkedSymAdjacent(center,
                    new ArrayList<Place>(region)));
            markAll(region, 1);
        }
        markAll(region, 0);
        return region;
    ...
    void toggleBoundary(int x, int y) {
        boundaryboard[x][y] = !boundaryboard[x][y];
    }
    ...
    Place opposing(Place p0, Place p) {
        int length = 2 * p0.x - p.x;
        int width = 2 * p0.y - p.y;
        if (isCell(length, width)) {
            return (pl(length, width));
        } else {
            return null;
        }
    }
    ...
    List<Place> unmarkedContaining(Place place) {
        if (isCell(place)) {
            ...
            if (markboard[place.x - 1][place.y ] == 0
                    && markboard[place.x + 1][place.y] == 0) {
                return asList(place.move(-1, 0),
                        place.move(1, 0));
            }
        } else if (isHoriz(place)) {
            if (markboard[place.x][place.y - 1] == 0
                    && markboard[place.x][place.y + 1] == 0) {
                return asList(place.move(0, -1),
                        place.move(0, 1));
            }
        } else {
            for (int i = 0; i < 4; i += 1) {
                if ((mark(place) == 0 || mark(place) == -1)
                        && ((mark(place.move(-i, -i)) != 0
                                && mark(place.move(-i, -i)) != -1)
                                || (mark(place.move(+i, -i)) != 0
                                && mark(place.move(+i, -i)) != -1)
                                || (mark(place.move(-i, +i)) != 0
                                && mark(place.move(-i, +i)) != -1)
                                ...
                                && mark(place.move(+i, +i)) != -1))) {
                    return asList();
                } else if ((mark(place) == 0 || mark(place) == -1)
                        && mark(place.move(-i, -i)) == 0
                        && mark(place.move(+i, -i)) == 0
                        && mark(place.move(-i, +i)) == 0
                        && mark(place.move(+i, +i)) == 0) {
                    return asList(place.move(-i, -i), place.move(+i, -i),
                            place.move(-i, +i), place.move(+i, +i));
                }
            }
            return asList();
        }
    ...
    List<Place> unmarkedSymAdjacent(Place center, List<Place> region) {
        ArrayList<Place> adjacent =
                new ArrayList<>();
        ArrayList<Place> result =
                new ArrayList<>();
        for (Place r : region) {
            assert isCell(r);
            if (isCell(r.move(-2, 0))) {
                adjacent.add(r.move(-2, 0));
            }
            if (isCell(r.move(+2, 0))) {
                adjacent.add(r.move(+2, 0));
            }
            if (isCell(r.move(0, +2))) {
                adjacent.add(r.move(0, +2));
            }
            if (isCell(r.move(0, -2))) {
                adjacent.add(r.move(0, -2));
            }
        }
        for (Place s : adjacent) {
            if (mark(s) == 0) {
                if (opposing(center, s) != null
                        && isCell(opposing(center, s))
                        && mark(opposing(center, s)) == 0) {
                    result.add(s);
                }
            }
        }
